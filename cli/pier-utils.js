...
const { DEFAULT_REGISTRY_PATH } = require("../server/workspace-registry");
...
function normalizeConfig(raw) {
  ...
  return {
    ...
    bridge: {
      ...
      token: typeof bridge.token === "string" && bridge.token.trim() ? bridge.token.trim() : randomToken(),
      defaultCwd:
        typeof bridge.defaultCwd === "string" && bridge.defaultCwd.trim() ? path.resolve(bridge.defaultCwd) : os.homedir(),
      shell: typeof bridge.shell === "string" && bridge.shell.trim() ? bridge.shell.trim() : null,
      workspaceRouteMapPath:
        typeof bridge.workspaceRouteMapPath === "string" && bridge.workspaceRouteMapPath.trim()
...
async function waitForBridgeHealthy(config, { attempts = 20, intervalMs = 250 } = {}) {
  let last = null;
