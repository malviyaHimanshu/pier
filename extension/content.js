(() => {
  ...
  function ensureTerminal(settings) {
    ...
    terminal.writeln("[terminal.browser] Press Ctrl+` (or Cmd+`) to toggle this panel.");
  ...
  function updateWebglMode(preferWebgl) {
    ...
    if (!preferWebgl) {
      ...
      if (webglStatus !== "disabled") {
        ...
        terminal.writeln("[terminal.browser] WebGL renderer disabled (using default renderer).");
      }
      return;
    }

    if (webglAddon || webglStatus === "enabled") {
      return;
    }
    ...
    try {
      ...
      if (typeof webglAddon.onContextLoss === "function") {
        webglAddon.onContextLoss(() => {
          webglStatus = "failed";
          terminal.writeln("[terminal.browser] WebGL context lost. Falling back to default renderer.");
        });
      }
    } catch (error) {
      webglStatus = "failed";
      terminal.writeln(`[terminal.browser] WebGL unavailable (${String(error.message || error)}), using default renderer.`);
      webglAddon = undefined;
    }
  }
