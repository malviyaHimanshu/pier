...
function ensureRegistryShape(raw) {
  ...
  for (const [hostKey, record] of routeEntries) {
    try {
      ...
      if (typeof recordObj.cwd !== "string" || !recordObj.cwd.trim()) {
        continue;
      }
      routes[host] = {
        cwd: String(recordObj.cwd),
        createdAt:
        ...
        updatedAt:
          typeof recordObj.updatedAt === "string"
            ...
            : undefined,
        source:
          typeof recordObj.source === "string" ? recordObj.source : undefined,
...
function listRoutes({ filePath = DEFAULT_REGISTRY_PATH } = {}) {
  ...
  return Object.entries(registry.routes)
    .map(([host, record]) => ({ host, ...((record as any) || {}) }))
