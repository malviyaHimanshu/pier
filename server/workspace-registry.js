...
function normalizeRouteHost(value, { allowLoopbackLiteral = true } = {}) {
  ...
  if (raw.includes("://")) {
    ...
    raw = String(parsed.hostname || "").trim().toLowerCase();
  }
  ...
  if (isLoopbackLiteral(raw)) {
    if (!allowLoopbackLiteral) {
      throw new Error("Loopback literal hosts are not allowed here. Use a .localhost hostname.");
    }
    return raw;
  }
  ...
  if (!withoutSuffix) {
    throw new Error(`Host must include a name before ${LOCALHOST_SUFFIX}: ${value}`);
  }
...
function ensureRegistryShape(raw) {
  ...
  const routes = {};
  const routeEntries = base.routes && typeof base.routes === "object" ? Object.entries(base.routes) : [];
...
function upsertRoute({ host, cwd, source, notes, filePath = DEFAULT_REGISTRY_PATH }) {
  ...
  registry.routes[normalizedHost] = {
    ...
    updatedAt: now,
    source: typeof source === "string" && source.trim() ? source.trim() : existing.source,
...
function resolveWorkspaceForHost(host, { filePath = DEFAULT_REGISTRY_PATH } = {}) {
  const normalizedHost = normalizeRouteHost(host);
  const registry = readRegistry(filePath);
