...
export default function ContributingDocsPage() {
  return (
    <DocsPageShell slug="contributing">
      <h2 className="text-2xl sm:text-3xl font-medium text-neutral-900 leading-tight">Prerequisites</h2>
      ...
      <ul className="list-disc pl-5 space-y-2 text-neutral-600">
        <li>
          Author extension code in <InlineCode>packages/extension-src</InlineCode>
        </li>
        <li>
          Do not edit files in <InlineCode>extension/</InlineCode> directly
        </li>
        <li>
          <InlineCode>extension/</InlineCode> is generated build output (ignored in git)
        </li>
        <li>
          Node package runtime output lives in <InlineCode>packages/*/dist</InlineCode>
          {' '}
          (ignored in git)
        </li>
      </ul>

      <h2 className="mt-10 text-2xl sm:text-3xl font-medium text-neutral-900 leading-tight">Local Setup</h2>
      <CommandBlock>{"pnpm install\npnpm run build\npnpm run check"}</CommandBlock>
      ...
      <ul className="list-disc pl-5 space-y-2 text-neutral-600">
        <li>
          <InlineCode>pnpm run build</InlineCode>: compile packages, build extension, sync
          manifest
        </li>
        <li>
        ...
        <li>
          <InlineCode>pnpm run test:unit</InlineCode>: package build and Vitest suite
        </li>
        <li>
          <InlineCode>pnpm run test:smoke</InlineCode>: smoke validation for built assets and
          CLI shims
        </li>
        <li>
          <InlineCode>pnpm run test:pack-smoke</InlineCode>: tarball install smoke test
        </li>
      </ul>
